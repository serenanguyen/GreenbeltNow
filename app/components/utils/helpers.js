  ...
  getWaterData(locationID, response, waterData) {
    ...
    if (resultID === locationID) {
      ...
      if (waterData.discharge > 5 && 2 < waterData.waterLevel < 5) {
        waterData.overallCondition =
          "Water level is a little low, swim with caution!";
      } else if (waterData.discharge > 5 && waterData.waterLevel > 5) {
        waterData.overallCondition =
          "Water conditions look great! Have fun swimming!";
      } else {
        waterData.overallCondition =
          "Conditions aren't optimal. Try Barton Springs Pool or Deep Eddy Pool instead!";
      }
    }
